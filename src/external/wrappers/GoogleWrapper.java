	...
	public Ranking search(String query, Proxy proxy) {
		...
		if (r != null) {
			...
			try {
				...
				if (proxy != null) {
					doc = Jsoup.connect("https://google.com/search?q=" + query2 + "&num=" + RESULTS).proxy(proxy)
							.userAgent(USER_AGENT).timeout(0).get();
				} else {
					doc = Jsoup.connect("https://google.com/search?q=" + query2 + "&num=" + RESULTS)
							.userAgent(USER_AGENT).timeout(0).get();
				}
				...
				for (Element result : doc.select("h3.r a")) {
					...
					try {
						...
						tech.outputSettings(new Document.OutputSettings().prettyPrint(false));// makes
																								// html()
																								// preserve
																								// linebreaks
																								// and
